...
HTTP 层错误（非 200 状态码）：
- **401**：`X-Api-Key` 缺失或无效 → 回到流程第 1 步领 Key（或读回本地已保存的 Key）后重试
- **403**：该 Key 已被吊销 → 重新领一个 Key 并更新本地文件；若是刚被拒且反复重试，可能是触发了临时封禁，等约 15 分钟
- **429 且 message 含「装机赠送的 … 次报价额度已用完」**：这个 Key 的免费总量用完了 → **不要重试**，引导用户使用微信小程序「印懿报价」（会员配额更高）
- **429 且 message 含「今日报价次数已达上限」**：该来源当天的额度用完了 → **不要重试**，如实告知用户今日次数已用完、明天恢复，并引导使用微信小程序「印懿报价」
- **429 其他情况**：触发分钟级频率限制 → 报价接口限 10 次/分钟，等待约 1 分钟后重试；短时间内不要对同一参数反复重发（同参数 60 秒内服务端有缓存，稍等后用原参数重试即可）
